...
const connectionString =
  process.env.DATABASE_URL || 'postgresql://localhost:5432/fruit_basket_app';
...
describe('The fruit basket service', function () {
  ...
  it('should be able to add 2 fruits to a basket', async function () {
    ...
    const basketId = await fruitBasket.createBasket(
      'My test basket for fruits'
    );
    assert.ok(basketId, 'The basket id should be set');

    const fruits = await fruitBasket.listFruits();

    const apples = fruits[0].id;
    const bananas = fruits[1].id;

    await fruitBasket.addFruitToBasket(apples, basketId, 3);
    await fruitBasket.addFruitToBasket(bananas, basketId, 7);
